...
<#if roleFullName?starts_with("doris-fe") >
        readinessProbe:
          exec:
            command:
            - /bin/sh
            - -c
            - ps aux | grep DorisFE | grep -v grep > /dev/null
          failureThreshold: 3
        ...
        resources:
          requests:
            memory: "${conf['doris.fe.container.request.memory']}Mi"
            cpu: "${conf['doris.fe.container.request.cpu']}"
          limits:
            memory: "${conf['doris.fe.container.limit.memory']}Mi"
            cpu: "${conf['doris.fe.container.limit.cpu']}"
</#if>
<#if roleFullName?starts_with("doris-be") >
        readinessProbe:
          httpGet:
            path: /metrics
        ...
        resources:
          requests:
            memory: "${conf['doris.be.container.request.memory']}Mi"
            cpu: "${conf['doris.be.container.request.cpu']}"
          limits:
            memory: "${conf['doris.be.container.limit.memory']}Mi"
            cpu: "${conf['doris.be.container.limit.cpu']}"
</#if>
        env:
          - name: NODE_NAME
            valueFrom:
          ...
          - name: ROLE_FULL_NAME
            value: "${roleFullName}"
          - name: MASTER_FE_IP
            value: "${serviceRoles['DORIS_FE_MASTER'][0]["ip"]}"
          - name: FE_QUERY_PORT
...
<#list dataPathList as dataPath>
      ...
      - name: service-render
        configMap:
          name: doris-service-render
      - name: service-common
        configMap:
          name: doris-service-common
      - name: "workspace"
        hostPath:
