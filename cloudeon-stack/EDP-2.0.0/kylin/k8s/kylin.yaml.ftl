...
<#if dependencies.FILEBEAT??>
    ...
    <#--          filebeat test output-->
      ...
      - image: "${conf['serverImage']}"
        ...
        resources:
          requests:
            memory: "${conf['kylin.container.request.memory']}Mi"
            cpu: "${conf['kylin.container.request.cpu']}"
          limits:
            memory: "${conf['kylin.container.limit.memory']}Mi"
            cpu: "${conf['kylin.container.limit.cpu']}"
        ports:
          - name: http
            protocol: TCP
            containerPort: ${conf['kylin.ui.port']}
        env:
          - name: NODE_NAME
            valueFrom:
              fieldRef:
...
<#list dataPathList as dataPath>
      ...
      - name: service-render
        configMap:
          name: kylin-service-render
      - name: service-common
        configMap:
          name: kylin-service-common
      - name: "workspace"
        hostPath:
