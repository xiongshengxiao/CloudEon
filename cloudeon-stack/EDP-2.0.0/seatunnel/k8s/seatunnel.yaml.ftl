...
<#case "seatunnel-master">
        ...
        resources:
          requests:
            memory: "${conf['seatunnel.master.container.request.memory']}Mi"
            cpu: "${conf['seatunnel.master.container.request.cpu']}"
          limits:
            memory: "${conf['seatunnel.master.container.limit.memory']}Mi"
            cpu: "${conf['seatunnel.master.container.limit.cpu']}"
<#break>
<#case "seatunnel-worker">
        readinessProbe:
          httpGet:
            path: /hazelcast/rest/instance/metrics
        ...
        resources:
          requests:
            memory: "${conf['seatunnel.worker.container.request.memory']}Mi"
            cpu: "${conf['seatunnel.worker.container.request.cpu']}"
          limits:
            memory: "${conf['seatunnel.worker.container.limit.memory']}Mi"
            cpu: "${conf['seatunnel.worker.container.limit.cpu']}"
<#break>
    <#case "seatunnel-web">
        readinessProbe:
          httpGet:
        ...
        resources:
          requests:
            memory: "${conf['seatunnel.web.container.request.memory']}Mi"
            cpu: "${conf['seatunnel.web.container.request.cpu']}"
          limits:
            memory: "${conf['seatunnel.web.container.limit.memory']}Mi"
            cpu: "${conf['seatunnel.web.container.limit.cpu']}"
<#break>
</#switch>
        env:
          - name: NODE_NAME
...
<#list dataPathList as dataPath>
      ...
      - name: service-render
        configMap:
          name: seatunnel-service-render
      - name: service-common
        configMap:
          name: seatunnel-service-common
      - name: "workspace"
        hostPath:
